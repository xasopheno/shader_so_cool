...
fn fs_main(
    ...
    var thing = sin(uv.x * 10.0) < 0.5;
    if (thing) {
        return vec4<f32>(sin(u.time + 0.5) * 0.1, sin(1.0/uv.y * 3.0 * u.time), 0.001, sin(0.001 * u.time));
    } else {
        return vec4<f32>(sin(u.time + 0.5) * 0.1, 0.001, 0.001, sin(u.time * 4.0));
    }
